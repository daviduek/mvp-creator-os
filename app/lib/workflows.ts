...
export function buildNsfwEditGraph(opts: {
  ...
  // KNOWN-GOOD NSFW pipeline: CR Pony + LoRA + IP-Adapter FaceID img2img.
  // (Lustify+ReActor variant made workers unhealthy; reverted to this tested path.)
  const prompt = ensureTrigger(opts.prompt);
  const seed = opts.seed ?? randomSeed();
  ...
  return {
    ...
    '10': {
      class_type: 'LoraLoader',
      inputs: {
        lora_name: LORA_NAME, strength_model: opts.lora_weight, strength_clip: opts.lora_weight,
        model: ['1', 0], clip: ['1', 1],
      },
    },
